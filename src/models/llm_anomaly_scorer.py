...
class QwenAdapter:
    def __init__(self, model_dir, tokenizer_path, max_seq_len, batch_size):
        self.model_dir = model_dir
        self.model_type = "qwen"
        self.tokenizer = AutoTokenizer.from_pretrained(
            tokenizer_path if tokenizer_path else model_dir,
    ...
    def chat_completion(self, dialogs, max_gen_len, temperature, top_p):
        results = []
        for dialog in dialogs:
            ...
            results.append({"generation": {"content": response}})
...
class LLMAnomalyScorer:
    ...
    def _generate_temporal_summaries(self, video, video_captions):
        ...
        for batch_start_frame in tqdm(
            ...
            for result, clip_frame_idx in zip(results, batch_frame_idxs):
                ...
                temporal_summaries[str(clip_frame_idx)] = content.split(
                    "\n"
                )[-1].strip()
    ...
    def _interpolate_unmatched_scores(self, scores):
        ...
        if len(valid_scores) == 1:
            return {k: valid_scores[0][1] for k in scores.keys()}
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="llama",
        choices=["llama", "qwen"],
        help="Model type: 'llama' for Llama 2, 'qwen' for Qwen-7B.",
    )
