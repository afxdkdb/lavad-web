...
ucf_crime_dir="/home/jinanyang/lavad_new/ucf_crime"
...
VENV_DIR="/home/jinanyang/miniconda/etc/profile.d/conda.sh"
...
# For Qwen model:
torchrun \
    --nproc_per_node 1 --nnodes 1 -m src.models.llm_anomaly_scorer \
    --root_path "$root_path" \
    --annotationfile_path "$annotationfile_path" \
    --batch_size "$batch_size" \
    --frame_interval "$frame_interval" \
    ...
    --output_summary_dir "$output_summary_dir" \
    --captions_dir "$captions_dir" \
    --ckpt_dir ~/models/Qwen-7B-Chat/ \
    --tokenizer_path ~/models/Qwen-7B-Chat/ \
    --model_type qwen \
    --num_jobs 4 \
